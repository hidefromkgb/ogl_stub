...
ENGC *cMakeEngine() {
    ...
    ENGC *retn = calloc(1, sizeof(*retn));

    retn->fang = (VEC_T2FV){{30.00 * VEC_DTOR, 30.00 * VEC_DTOR}};
    retn->ftrn = (VEC_T3FV){{3.4, -3.8, -6.0}};
    MakeCube(&attr[0], &attr[1], 2.0);
    _VBO(retn->cube, 1, GL_TRIANGLES, attr, ((OGL_UNIF[])
       {{.name = "mMVP", .type = OGL_UNI_TMFV, .pdat = &retn->view},
        {.name = "tile", .type = OGL_UNI_T1II, .pdat = (GLvoid*)0}}),
       ((char*[]) { /** === vertex shader **/
        "uniform mat4 mMVP; attribute vec3 vert; varying vec3 vpos;"
        "void main() { gl_Position = mMVP * vec4(vpos = vert, 1.0); }",
        "uniform sampler2D tile; varying vec3 vpos;" /** === pixel shader **/
        "void main() { vec3 ctex;"
                 "if (abs(vpos.x) > 0.999) ctex = vpos.yzx;"
            "else if (abs(vpos.y) > 0.999) ctex = vpos.zxy;"
            "else if (abs(vpos.z) > 0.999) ctex = vpos.xyz;"
            "gl_FragColor = texture2D(tile, ctex.xy * ctex.z * 0.5)"
                         "/ length(ctex); }"}));
